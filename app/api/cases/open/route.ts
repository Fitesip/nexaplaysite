...
import { caseRewardCredits } from "@/lib/caseReward";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (compensationAmount > 0 || rubleAmountKopecks > 0) {
      await conn.query(
        `UPDATE users
         SET game_currency = game_currency + ?, balance_kopecks = balance_kopecks + ?
         WHERE id = ?`,
        [compensationAmount, rubleAmountKopecks, userId]
      );
    }
