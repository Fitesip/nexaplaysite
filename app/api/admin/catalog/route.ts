...
export async function GET() {
  ...
  const [rows]: any = await pool.query(
    `SELECT id, name, category, game_mode, price_rub AS price, stock, hidden, one_time_purchase, is_case, description, created_at
     FROM catalog_items
...
export async function POST(req: NextRequest) {
  ...
  const { name, category, gameMode, price, description, stock, hidden, oneTimePurchase, isCase } = parsed.data;

  const pool = getPool();
  const [result]: any = await pool.query(
    `INSERT INTO catalog_items (name, category, game_mode, price_rub, stock, hidden, one_time_purchase, is_case, description)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [name, category, gameMode, price, stock ?? null, hidden ? 1 : 0, oneTimePurchase ? 1 : 0, isCase ? 1 : 0, description]
  );
